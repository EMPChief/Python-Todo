...
class FileOperations:
    ...
    def update_tasks_file(tasks):
        try:
            ...
            sg.PopupError(
                f"Error saving data to file: {e}", title="File Error")
...
class TaskOperations:
    ...
    def edit_task(tasks, window, task_list):
        ...
        if task_index:
            ...
            sg.Popup(
                f"Task updated from '{old_task_description}' to '{new_task_description}'", title="Task Updated")
            FileOperations.update_tasks_file(tasks)
            window['-TASK-'].update('')
            TaskOperations.display_tasks(tasks, window)
    ...
    def remove_task(tasks, window, task_list):
        ...
        if task_index:
            ...
            sg.Popup(
                f"Task '{removed_task['description']}' removed", title="Task Removed")
            FileOperations.update_tasks_file(tasks)
            TaskOperations.display_tasks(tasks, window)
    ...
    def complete_task(tasks, window, task_list):
        ...
        if task_index:
            ...
            if task_to_complete["status"] == "complete":
                sg.Popup(
                    f"Task '{task_to_complete['description']}' is already marked as complete", title="Task Completed")
            else:
                task_to_complete["status"] = "complete"
                sg.Popup(
                    f"Task '{task_to_complete['description']}' marked as complete", title="Task Completed")
                FileOperations.update_tasks_file(tasks)
                TaskOperations.display_tasks(tasks, window)
    ...
    def display_tasks(tasks, window):
        task_list = [
            f"{task['description']} | Status: {task['status']}" for task in tasks]
