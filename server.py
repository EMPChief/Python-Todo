    ...
    def edit_task(index, new_description):
        try:
            ...
            if index >= 0 and index < len(tasks):
                ...
                FileOperations.update_tasks_file(tasks)
                flash(
                    f"Task updated from '{old_description}' to '{new_description}'", 'success')
            else:
    ...
    def remove_task(index):
        try:
            ...
            if index >= 0 and index < len(tasks):
                ...
                FileOperations.update_tasks_file(tasks)
                flash(
                    f"Task '{removed_task['description']}' removed.", 'success')
            else:
                flash('Invalid task index.', 'danger')
        except Exception as e:
            flash(f'Error: {str(e)}', 'danger')
        ...
        try:
            ...
            if index >= 0 and index < len(tasks):
                ...
                if task_to_complete["status"] == "complete":
                    flash(
                        f"Task '{task_to_complete['description']}' is already marked as complete.", 'info')
                else:
                    task_to_complete["status"] = "complete"
                    FileOperations.update_tasks_file(tasks)
                    flash(
                        f"Task '{task_to_complete['description']}' marked as complete.", 'success')
            else:
                flash('Invalid task index.', 'danger')
        except Exception as e:
            flash(f'Error: {str(e)}', 'danger')
